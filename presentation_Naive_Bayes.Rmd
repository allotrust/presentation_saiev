...
library(RWeka)         # For bigram tokenization

# Load the dataset from a CSV file
my_data <- read.csv("spam.csv", encoding = "ISO-8859-1")
...
# Define a function to tokenize text into bigrams (pairs of words)
BigramTokenizer <- function(x) {
  NGramTokenizer(x, Weka_control(min = 2, max = 2))
}

# Create a Document-Term Matrix (DTM) with TF-IDF weighting and bigram tokenization
dtm <- DocumentTermMatrix(corpus, control = list(
  weighting = weightTfIdf,     # Use TF-IDF weighting
  tokenize = BigramTokenizer   # Apply bigram tokenization
))
